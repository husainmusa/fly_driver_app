    ...
    this.platform.ready().then(() => {
      ...
      if (!lng || lng === null) {
        ...
        this.api.post('users/getDefaultSettingsById', param).subscribe((data: any) => {
          ...
          console.log(this.util.translations);
          console.log(this.util.direction, "cside");
          console.log(this.util.cside, "cside");
          console.log(this.util.appClosed);
          console.log(this.util.appClosedMessage);
      ...
      this.util.subscribeLoggedIn().subscribe((s:any)=>{                
        if(this.util.userInfo && this.util.userInfo.id && this.util.userInfo.id!=""){
            this.firebaseDB.object(['drivers',this.util.userInfo.id,'status'].join('/')).snapshotChanges().subscribe((action)=>{
